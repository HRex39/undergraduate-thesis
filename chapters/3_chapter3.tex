...
\section{基于DQN算法的决策器设计} % 3.1 决策任务
...
\begin{figure}[htbp]
    ...
    \caption{highway-env}\label{highway-env} % label 用来在文中索引
\end{figure}  
...
反向数据更新指的是DQN网络根据经验回放库中的数据，对DQN网络参数进行的更新。根据2.3节所示的DQN算法，DQN网络需要构建主网络（$Q_{next}$）和TD网络（$Q_{target}$），并且需要通过两者差值的平方作为损失函数进行反向传递。从经验回放库中随机采样得到数据，对每组数据的执行如下操作：
...
\begin{figure}[htbp]
    ...
    \caption{metadrive}\label{metadrive} % label 用来在文中索引
\end{figure}  
